...
(def env :prod)
(def config (env (edn/read-string (slurp "binance.config.edn"))))
(def SYMBOL "BTCUSDT")
(def trade-client (.createTrade (SpotClientImpl. (:apiKey config) (:secret config) (:url config))))
(def market-client (.createMarket (SpotClientImpl. (:url config))))
